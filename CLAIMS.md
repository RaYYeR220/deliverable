...
| R48 | "There is no mock oracle: the only Scope account the tests accept is the real dump ... The program, the SDK and the tests use real mints only. The one stand-in token in the repository is in `market/` ... a Token-2022 stand-in with 8 decimals (`AAPLd`). Everything that reads or targets mainnet uses the real mints." | `REPRODUCIBLE` | `grep -rni "update_price\|mock" programs/` finds only the comment in `tests/mod.rs` that says there is none. `security_test::an_oracle_account_from_the_wrong_program_is_refused` copies the real Scope account under a new owner and asserts it is rejected, and `security_test::a_correctly_owned_scope_account_at_the_wrong_address_is_refused` copies it **byte for byte under the real Scope owner, to another address** and asserts that the rail refuses to publish from it and the gate records `OracleUnreadable`. That second test exists because the evidence for this claim did not used to cover the hole: `plant_impostor_oracle` only ever rewrote the **owner**, and `observe` checked only the owner, so a Scope-owned account that was not the bound feed was an acceptable price source at any index — the 2026-09-22 internal audit (F-01) found five live `OraclePrices` feeds and 150 indices populated in the bound one and simultaneously fresh in a sibling with unrelated prices. `oracle::observe` now binds the account by address, and `oracle::scope::decode` checks the discriminator and the exact length. The only quote-mint creation in the repository is `market/src/devnet-quote.ts` (`createInitializeMintInstruction`, `DECIMALS`), recorded in `market/artifacts/devnet-quote.json`. The mainnet series flow quotes in AAPLx and has been simulated, not sent (PROOF.md, Deployments). |
...
| M10 | The calendar covers a twelve-month window, committed from a schedule published in `MMDD` form | `REPRODUCIBLE` | `US_EQUITY_2026_2027` in `state/registry.rs`, compared with the publisher by verify-onchain 10. The entries are **stored year-scoped**, `(year << 16) | (month << 8) | day`, even though the publisher's form carries no year: keyed bare, Labor Day 2026 (7 September) refused an ordinary Tuesday in 2027 and Labor Day 2027 (the 6th) read as a regular session. `audit_test::the_committed_calendar_misreads_the_following_year` holds both directions. |
